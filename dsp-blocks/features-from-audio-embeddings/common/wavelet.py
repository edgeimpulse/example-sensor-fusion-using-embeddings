...
def calculate_statistics(x):
    output = {}
    output['n5'] = np.nanpercentile(x, 5)
    output['n25'] = np.nanpercentile(x, 25)
    output['n75'] = np.nanpercentile(x, 75)
    output['n95'] = np.nanpercentile(x, 95)
    output['median'] = np.nanpercentile(x, 50)
    output['mean'] = np.nanmean(x)
    output['std'] = np.nanstd(x)
    output['var'] = np.nanvar(x, ddof=1)
    output['rms'] = np.sqrt(np.nanmean(x**2))
